...
def render(width, height, ax, ay):
    ...
    for face in CUBE_FACES:
        ...
        # ``-dot`` flips because back-facing normals have positive nz and the
        # light has negative z too; for front faces dot is positive, so the
        # value below is already a usable brightness in [0, ~1].
        brightness = max(0.0, min(1.0, dot))
        face_info.append((face, brightness))
    ...
    # Rasterize a triangle with barycentric coordinates.
    def draw_triangle(p0, p1, p2, char):
        ...
        for y in range(min_y, max_y + 1):
            for x in range(min_x, max_x + 1):
                ...
                if (w0 >= 0 and w1 >= 0 and w2 >= 0) or (
                    ...
                    inv = 1.0 / area
                    z = z0 * (w0 * inv) + z1 * (w1 * inv) + z2 * (w2 * inv)
                    if z > zbuf[y][x]:
                        zbuf[y][x] = z
                        buf[y][x] = char

    for face, brightness in face_info:
        idx = min(len(SHADE_CHARS) - 1, int(brightness * len(SHADE_CHARS)))
        ch = SHADE_CHARS[idx] if brightness > 0.05 else "."
        p = [projected[i] for i in face]
        # Split quad into two triangles
        draw_triangle(p[0], p[1], p[2], ch)
        draw_triangle(p[0], p[2], p[3], ch)
